...
import { colors, colorsToEmoji } from "../Keyboard/constants";
...
const GuessDistributionLine = ({position, amount, percentage }) => {
    return (
        <View style = {{ flexDirection: "row", alignItems: "center", width: "100%" }}>
            ...
            <View style = {{ alignSelf: "stretch", backgroundColor: colors.grey, margin: 5, padding: 5, width: `${percentage}%`, minWidth: 20, }}>
...
const GuessDistribution = ({ distribution }) => {
    ...
    <View style = {{ width: "100%", padding: 20 }}>  
        {distribution.map((dist, index) => {
            return (
                <GuessDistributionLine key = { index } position = {0} amount = {2} percentage = {(100 * dist) / sum} />
            )
        })}
    </View>
    </>
...
const EndScreen = ({won = false, rows, getCellBGColor }) => {
    const [secondsTillTomorrow, setSecondsTillTommorrow] = useState(0);
    ...
    useEffect(() => {
        const updateTime = () => {
            ...
            setSecondsTillTommorrow((tomorrow - now) / 1000);
        }
    ...
    return (
        <View style = {{ width: "100%", alignItems: "center" }}>
            <Text style = {styles.title}> 
                {won ? "Congrats!" : "Mah, try again tomorrow"}
            </Text>

            <Text style = {styles.subtitle} > STATISTICS</Text>
            <View style = {{ flexDirection: "row", marginBottom: 20 }}>
                <Number number = {played} label = {"Player"} />
            ...
            <View style = {{ flexDirection: "row", padding: 10 }}>
                <View style = {{ alignItems: "center", flex: 1 }}>
                    <Text style = {{ color: colors.lightgrey }}>Next WORDQUEST</Text>
...
{/* 
            <Animated.Text entering {SlideInLeft.springify().mass(0.5)} 
                style = {styles.title}> 
                {won ? "Congrats!" : "Mah, try again tomorrow"}
            </Animated.Text>
            ...
            <Animated.View entering = {SlideInLeft.delay(100).springify().mass(0.5)} style = {{ width: "100%" }}>
                <GuessDistribution distribution={distribution}/>
            </Animated.View>
