...
import { colors , CLEAR , ENTER , colorsToEmoji } from "../Keyboard/constants";
...
const Game = () => {
  ...
  const word = words[dayOfTheYear % words.length];
  ...
  const persistState = async () => {
    ...
    {
      console.log("Fail data to async storage", e);
    }
  };
      ...
      {/* <ScrollView style={styles.map}>
        ...
        {rows.map((row, i) => (
          ...
          {row.map((letter, j) => (
            <>
              {i < curRow && (
                <Animated.View entering = {FlipInEasyY.delay(j * 100)} 
                  key = {`cell-color-${i}-${j}`} style={getCellStyle(i, j)}> 
                  <Text style={styles.cellText}>{letter.toUpperCase()} </Text>
                </View>
              )}
              ...
              {i == curRow && !!letter && (
                <Animated.View entering = {ZoomIn} 
                  key = {`cell-active-${i}-${j}`} style={getCellStyle(i, j)}> 
                  <Text style={styles.cellText}>{letter.toUpperCase()} </Text>
                </Animated.View>
              )}
